...
#if HAL_WITH_IO_MCU
    if (iomcu_enabled) {
        iomcu.set_reversible_mask(chanmask);
    }
#endif
}
...
#if HAL_WITH_IO_MCU
    if (iomcu_enabled && iomcu_dshot) {
        iomcu.set_dshot_rate(dshot_rate, loop_rate_hz);
    }
#else
    (void)dshot_rate;
    (void)loop_rate_hz;
