...
# fmuv2 / pixhawk6c_mini: legacy full-tree BSP under archive/stray-bsp/ (CUAV v5 uses hwdef/common).
RTT_BSP_MAP = {
...
# Archived legacy full-tree BSP (see libraries/AP_HAL_RTT/archive/stray-bsp/README.md)
RTT_BSP_FMUV2_SRC = 'libraries/AP_HAL_RTT/archive/stray-bsp/rtt_bsp_fmuv2'
RTT_BSP_PIXHAWK6C_MINI_SRC = 'libraries/AP_HAL_RTT/archive/stray-bsp/rtt_bsp_pixhawk6c_mini'
...
def _deploy_fmuv2_bsp_if_needed(env):
    ...
    does not exist, copy from SRCROOT/libraries/AP_HAL_RTT/archive/stray-bsp/rtt_bsp_fmuv2 to
...
def _deploy_cuav_v5_bsp_if_needed(env):
    ...
    does not exist, copy from libraries/AP_HAL_RTT/hwdef/common (RTT_BSP_CUAV_V5_SRC).
    If the deploy target already exists, sync key mutable files so that changes
    to the common template (rtconfig.h, link.lds, board.c, etc.) are always reflected.
    """
    bsp_rel = RTT_BSP_MAP.get(getattr(env, 'BOARD', None))
