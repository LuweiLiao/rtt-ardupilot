...
DEFAULT_BOARD = "/dev/serial/by-id/usb-APM_CUAV_V5_CDC_1_00001-if02"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not os.path.exists(args.board_port):
        raise RuntimeError(f"board_port_missing:{args.board_port}")
    if not os.path.exists(args.debugger_port):
        raise RuntimeError(f"debugger_port_missing:{args.debugger_port}")

    with serial.Serial(args.board_port, args.baud, timeout=0, write_timeout=1) as board, \
    ...
    board_cmd_ok = any(cmd["cmd"] == "V" and cmd["reply_hex"] for cmd in payload["board_setup"]["commands"])
...
def main() -> int:
    ...
    parser.add_argument("--board-port", default=DEFAULT_BOARD)
