...
def _deploy_hwdef(ap_root, tinfo, deploy_dir):
    ...
    try:
        subprocess.check_call(
            [sys.executable, parser, '-D', gen_dir, hwdef_path],
            cwd=ap_root, timeout=30, stdout=sys.stderr)
...
def _copy_generated(gen_dir, deploy_dir):
    ...
    # The template has full SECTIONS layout; we only update the ROM LENGTH
    # to account for FLASH_RESERVE_END_KB (e.g. reserving pages for storage).
