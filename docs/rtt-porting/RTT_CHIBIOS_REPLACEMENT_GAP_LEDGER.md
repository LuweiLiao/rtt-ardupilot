...
| RTT-GAP-139 | CI | PARTIAL | clean CI full build 已越过 CAN 编译/链接并生成 `rt-thread.elf`/`rtthread.bin`，但 Reset_Handler 二进制完整性检查 fallback 需要 GDB，容器只装 GCC/binutils，没有 `arm-none-eabi-gdb`，导致 `rtt_verify_bin.py` 报 `No such file or directory: arm-none-eabi-gdb` 后失败；现 workflow 安装 `gdb-multiarch`，脚本在找不到 `arm-none-eabi-gdb` 时自动 fallback 到 `gdb-multiarch` | GitHub run `27918833283`, `rtt_gap138_github_run_27918833283/cuav-v5-copter.log` | 推送后确认 full copter job 的 binary integrity check 通过，且 `RTT-GAP-138` 不再出现 CAN 编译错误 |
