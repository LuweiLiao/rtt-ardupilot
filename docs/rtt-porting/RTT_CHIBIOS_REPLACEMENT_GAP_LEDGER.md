...
| RTT-GAP-140 | CI | PARTIAL | GitHub container 内 `set_app_descriptor.py` 仍在 SCons 构建期间打印 `git rev-parse failed: fatal: detected dubious ownership`，虽然 descriptor 写入和当前 workflow 均成功，但 hash 仍显示 `0x00000000`，版本/commit 元数据可能弱化；第一次 scoped `GIT_CONFIG_COUNT` fallback 远端无效，现改为脚本内部对同一 HOME 执行 `git config --global --add safe.directory <source_root>` 后重试，并更新单元测试覆盖该序列 | GitHub runs `27919122896`, `27919445368`, `rtt_gap140_github_run_27919445368/*.log` | 推送后确认 APP_DESCRIPTOR 输出真实 `git_hash`，不再出现 dubious ownership/hash=0 |
