...
| RTT-GAP-087 | Build | PARTIAL | GitHub `test_rtt.yml` 在 apt SCons 4.0.1 下把 `--v=ArduCopter` 误解析为内建 `--version`，已改为 CI 不传 `--v`、本地改用安全 `--vehicle` 长选项；full 与 l0_boot 本地路径均通过 | `rtt_gap087_ci_scope_20260621T210411Z`, `rtt_gap087_scons_vehicle_ci_full2_20260621T212053Z`, `rtt_gap087_l0_boot_staged2_20260621T212433Z` | 推送后等 GitHub Actions 远端 run 确认 `test_rtt` GREEN |
...
| RTT-GAP-135 | CI | PARTIAL | `issue/rtt-*` push 仍触发 copter/plane/rover/chibios/macos/cygwin/dds 等普通 ArduPilot 大矩阵，产生与 RTT 无关的失败并稀释专用验收；21 个普通 push workflow 已加入 `branches-ignore: issue/rtt-*` | GitHub run `27917986917..27917986962`, workflow YAML audit | 推送后确认下一次 RTT push 只保留 RTT 专用 workflow/必要跳过项 |
| RTT-GAP-136 | CI | PARTIAL | 远端 `test_rtt.yml` 已越过旧 SCons `--v` 歧义，但 CI 容器缺 `arm-none-eabi-gcc`，同时测试构建在 BSP SCons 未产出 bin/elf 时不会立刻失败；现 workflow 安装 ARM GCC/binutils，根 `SConstruct` 增加 `rtthread.bin`/`rt-thread.elf` 必需产物 gate | `rtt_gap136_ci_toolchain_full_20260621T214226Z`, `rtt_gap136_ci_toolchain_l0_20260621T214335Z` | 推送后确认 GitHub `test_rtt` 不再报 toolchain path 且两个 matrix job 通过 |
| RTT-GAP-137 | Build | OPEN | 同一工作树内并行运行多个相同 RTT target 的根 SCons 会共享 `build/rtt_deploy/cuav_v5/_hwdef_gen`，dry-run 曾在 `hwdef.h.tmp -> hwdef.h` rename 上互相踩踏 | 本轮并行 dry-run 观察 | 设计 target-level lock 或 per-invocation staging，避免 CI/本地并发踩 deploy 目录 |
| RTT-GAP-138 | Build | PARTIAL | clean CI full build 生成的 RTT `ap_config.h` 会强制 `#undef HAL_NUM_CAN_IFACES` 并设为 0，覆盖 CUAV V5 hwdef/命令行的 CAN=2，导致 `AP_CANManager.cpp` 找不到 `HAL_CANIface` 且 DroneCAN 把 iface 数组视作 0 长度；现模板只包含 `hwdef.h` 并清理旧错误覆盖段 | GitHub run `27918448978`, `rtt_gap138_can_apconfig_rebuild_20260621T215615Z` | 推送后确认 GitHub full copter job 不再出现 `HAL_CANIface`/`ifaces[0]` 失败 |
