...
| RTT-GAP-140 | CI | OPEN | GitHub container 内 `set_app_descriptor.py` 仍在 SCons 构建期间打印 `git rev-parse failed: fatal: detected dubious ownership`，虽然 descriptor 写入和当前 workflow 均成功，但 hash 仍显示 `0x00000000`，版本/commit 元数据可能弱化；应在 checkout 后、构建前更早设置 safe.directory 或让 descriptor 工具显式处理 CI workspace 所有权 | GitHub run `27919122896`, `rtt_gap139_github_run_27919122896/*.log` | 后续修复并确认 APP_DESCRIPTOR hash/version 元数据不再因 dubious ownership 降级 |
...
- 新发现 `RTT-GAP-140`：远端 run `27919122896` 的 L0/full 日志仍出现 `set_app_descriptor.py` 报 `git rev-parse failed: fatal: detected dubious ownership`。当前 workflow 不受影响且 APP_DESCRIPTOR 被写入成功，但 descriptor hash 显示 `0x00000000`，说明版本/commit 元数据仍可能降级；后续应把 `git config --global --add safe.directory "$GITHUB_WORKSPACE"` 提前到 checkout 后/构建前所有脚本可见，或让 descriptor 工具对 CI workspace 所有权做显式 fallback。
