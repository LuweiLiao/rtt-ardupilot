...
| RTT-GAP-140 | CI | PARTIAL | GitHub container 内 `set_app_descriptor.py` 仍在 SCons 构建期间打印 `git rev-parse failed: fatal: detected dubious ownership`，虽然 descriptor 写入和当前 workflow 均成功，但 hash 仍显示 `0x00000000`，版本/commit 元数据可能弱化；现 descriptor 脚本在遇到 dubious ownership 时用 scoped `GIT_CONFIG_COUNT` safe.directory 重试，且补充单元测试覆盖 fallback | GitHub run `27919122896`, `rtt_gap139_github_run_27919122896/*.log` | 推送后确认 APP_DESCRIPTOR 输出真实 `git_hash`，不再出现 dubious ownership/hash=0 |
...
- `RTT-GAP-140`：修复 CI APP_DESCRIPTOR git hash 降级路径。远端 run `27919122896` 的 L0/full 日志仍出现 `set_app_descriptor.py` 报 `git rev-parse failed: fatal: detected dubious ownership`，APP_DESCRIPTOR 虽然写入成功但 hash 为 `0x00000000`；即使 workflow build step 已执行 `git config --global --add safe.directory "$GITHUB_WORKSPACE"`，SCons/RTT post action 内部仍可能因 HOME/global config 时序不同而看不到该设置。现在 `set_app_descriptor.py` 在 `git rev-parse` 遇到 dubious ownership 时，用仅作用于该 git 子进程的 `GIT_CONFIG_COUNT/GIT_CONFIG_KEY_0/GIT_CONFIG_VALUE_0` 注入 `safe.directory=<source_root>` 后重试；新增 `Tools/scripts/build_tests/test_rtt_set_app_descriptor.py` 覆盖该 fallback，并让 `test_rtt.yml` paths 包含 RTT build test 文件。远端仍需推送后确认，因此主表保持 PARTIAL。
