...
| RTT-GAP-140 | CI | PARTIAL | GitHub container 内 `set_app_descriptor.py` 仍在 SCons 构建期间无法取得 git hash，最初为 dubious ownership，随后收敛为 RT-Thread SCons post action 环境 `$HOME not set`，导致 APP_DESCRIPTOR hash 仍为 `0x00000000`；现 descriptor 脚本为 git 创建临时 HOME，在该私有 HOME 中写入 `safe.directory=<source_root>` 后读取短 hash，并更新单元测试覆盖该路径 | GitHub runs `27919122896`, `27919445368`, `27919693436`, `rtt_gap140_github_run_27919693436/*.log` | 推送后确认 APP_DESCRIPTOR 输出真实 `git_hash`，不再出现 HOME/dubious/hash=0 |
...
- `RTT-GAP-140`：继续修复 CI APP_DESCRIPTOR git hash 降级路径。远端 run `27919122896` 的 L0/full 日志仍出现 `set_app_descriptor.py` 报 `git rev-parse failed: fatal: detected dubious ownership`，APP_DESCRIPTOR 虽然写入成功但 hash 为 `0x00000000`；即使 workflow build step 已执行 `git config --global --add safe.directory "$GITHUB_WORKSPACE"`，SCons/RTT post action 内部仍可能因 HOME/global config 时序不同而看不到该设置。第一次修复使用 `GIT_CONFIG_COUNT/GIT_CONFIG_KEY_0/GIT_CONFIG_VALUE_0` 对单个 git 子进程注入 safe.directory，并新增 `Tools/scripts/build_tests/test_rtt_set_app_descriptor.py` 接入 workflow；远端 run `27919445368` 证明该方式仍未消除 dubious ownership，hash 仍为 0。第二次改为 descriptor 脚本在检测到 dubious ownership 后，对当前 HOME 显式执行 `git config --global --add safe.directory <source_root>` 再重试；远端 run `27919693436` 进一步把根因收敛为 RT-Thread SCons post action 环境 `$HOME not set`，因此 `git config --global` 仍失败。现在改为 `git_short_hash()` 自己创建临时 HOME，并在该私有 HOME 中写入 `safe.directory=<source_root>` 后执行 `git -C <source_root> rev-parse --short HEAD`；单元测试同步覆盖“临时 HOME + safe.directory + rev-parse”路径。远端仍需推送后确认，因此主表保持 PARTIAL。
