...
| RTT-GAP-101 | Test | FIXED | `rtt_acceptance_suite.py` 已新增 manifest 驱动的 `--full-chain --rounds N` 固定入口，串联 manifest 校验、QGC/MP host evidence、loop-rate、参数下载、MAVFTP、外设、日志、可选 SocketCAN/DroneCAN、source artifact audit，并输出 capability coverage matrix | `Tools/scripts/rtt_acceptance_suite.py`, `test_rtt_acceptance_suite.py`, `rtt_gap101_manifest_acceptance_suite_20260622T000000Z/mock_full_chain_acceptance.json`, `script_tests.log`, GitHub run `27922719734`, `rtt_gap101_github_run_27922719734/*.log` | 后续实机总验收运行 `--full-chain --rounds N --include-socketcan --sudo-socketcan` 采集真硬件 N 轮 |
| RTT-GAP-102 | Test | FIXED | 已新增 QGC/Mission Planner 主机侧证据采集工具，汇总 QGC/MP 进程、X11 窗口、可选截图、USB CDC owner 诊断、日志/配置目录摘要，并明确区分“GUI 未运行/无窗口”和“USB 端口冲突” | `Tools/scripts/rtt_qgc_mp_evidence.py`, `test_rtt_qgc_mp_evidence.py`, `rtt_gap102_qgc_mp_evidence_20260622T000000Z/qgc_mp_evidence.json`, GitHub run `27921823379`, `rtt_gap102_github_run_27921823379/*.log` | QGC 打开后可复跑并追加截图/窗口证据，再接自动点击连接 gate |
...
| RTT-GAP-105 | Test | FIXED | 已新增 CUAV V5 RTT hardware capability manifest，机器可读声明 build/USB/MAVLink/SLCAN/CAN/IMU/Compass/Baro/SDCard/logging/QGC-host/workspace 能力、证据、边界，并新增校验器把 manifest 关键字段对齐 `hwdef.dat` | `docs/rtt-porting/manifests/cuav_v5_rtt_capabilities.json`, `Tools/scripts/rtt_capability_manifest_check.py`, `test_rtt_capability_manifest_check.py`, `rtt_gap105_capability_manifest_20260622T000000Z/capability_manifest_check.json`, GitHub run `27922287434`, `rtt_gap105_github_run_27922287434/*.log` | 后续 acceptance suite 可读取该 manifest 生成必测矩阵 |
...
- `RTT-GAP-101`：把 `rtt_acceptance_suite.py` 从固定 4 个 MAVLink gate 扩展为 manifest 驱动的全链路入口。新增 `--full-chain` 后会顺序运行 capability manifest check、QGC/Mission Planner host evidence、loop-rate、参数下载、MAVFTP、peripheral driver、日志下载、可选 SocketCAN/DroneCAN、source artifact audit；新增 `--rounds N` 串行多轮执行；新增 manifest coverage matrix，把每个 `required_for_chibios_replacement` capability 标成 `covered_passed`、`covered_failed`、`not_covered`、`manifest_static_evidence` 或 `manifest_boundary`。默认不带 `--full-chain` 时保留旧 MAVLink-only 行为，避免破坏已有脚本。验证：新增 mock 单测覆盖 full-chain 全覆盖、未启用 SocketCAN 时 CAN required capability 报 missing、`--rounds 2` 串行执行；RTT build_tests 共 18 个单测 OK；离线 mock full-chain 证据 `mock_full_chain_acceptance.json` 显示 `missing_required=[]`、`failed_required=[]`、`boundary_required=[prearm_flight_readiness]`，清楚保留飞行级 pre-arm 边界；源码树 artifact audit 为 0。远端 run `27922719734` 两个 matrix job 均 GREEN，日志显示 `Ran 18 tests` / `OK`，full 仍通过 `Binary integrity check PASSED`，APP_DESCRIPTOR hash 为 `0x0e99f830`，未出现旧 SCons/CAN/hash=0 错误。
- `RTT-GAP-105`：新增 `docs/rtt-porting/manifests/cuav_v5_rtt_capabilities.json`，把 CUAV V5 RTT 的替代 ChibiOS 所需能力做成机器可读契约：SCons full/L0 构建、400Hz 主循环、USB MAVLink if00、快速参数下载、MAVFTP+SDCard、USB SLCAN if02、SocketCAN 标准/扩展帧、pydronecan NodeStatus、IMU/INS、磁力计、气压计、DataFlash 日志、QGC/MP host 证据、工作区 artifact audit，以及 pre-arm/飞行级边界。新增 `Tools/scripts/rtt_capability_manifest_check.py` 校验 manifest schema、必需能力、状态枚举、USB if00/if02 角色，并把 `BOARD_NAME/MCU/SERIAL_ORDER/DEFAULT_SERIAL0_BAUD/HAL_OTG2_PROTOCOL/APJ_BOARD_ID` 等关键项对齐 `libraries/AP_HAL_RTT/hwdef/cuav_v5/hwdef.dat`，防止文档和 hwdef 漂移。验证：`capability_manifest_check.json` 为 GREEN、`capability_count=17`；RTT build_tests 共 15 个单测 OK；源码树 artifact audit 为 0。远端 run `27922287434` 两个 matrix job 均 GREEN，日志显示 `Ran 15 tests` / `OK`，full 仍通过 `Binary integrity check PASSED`，APP_DESCRIPTOR hash 为 `0x0e891e10`，未出现旧 SCons/CAN/hash=0 错误。
- `RTT-GAP-102`：新增 `Tools/scripts/rtt_qgc_mp_evidence.py` 作为 QGC/Mission Planner 主机侧证据采集入口。脚本只读采集，不自动点击、不 reset USB、不杀进程；它复用 `rtt_usb_port_conflict_diag.py` 汇总 `/dev/ttyACM*` owner/role，同时采集 `pgrep` 进程、`wmctrl`/`xdotool` X11 窗口、可选 `import` 截图、QGC/MP 常见日志与配置目录。单元测试覆盖窗口解析、过滤 `pgrep` 自身噪声、USB RED 向上升级、GUI 未运行时输出 YELLOW 而不是假失败。当前本机证据 `rtt_gap102_qgc_mp_evidence_20260622T000000Z/qgc_mp_evidence.json` 显示 USB 为 GREEN：`/dev/ttyACM1` 是 `rtt_mavlink_cdc`、`/dev/ttyACM2` 是 `rtt_slcan_cdc`，均未被占用；总 verdict 为 YELLOW，仅因为当前没有 QGC/MP 进程和窗口，因此本轮没有声称 GUI 连接验收通过。远端 run `27921823379` 两个 matrix job 均 GREEN，日志显示 `Ran 11 tests` / `OK`，full 仍通过 `Binary integrity check PASSED`，APP_DESCRIPTOR hash 为 `0x02a2b131`，未出现旧 SCons/CAN/hash=0 错误。
...
- `RTT-GAP-104`：新增 `Tools/scripts/rtt_openocd_guard.py`，把 OpenOCD 清理从多个脚本里的 ad-hoc `pkill` 收敛成统一守卫。新 guard 先用 exact `pkill -x openocd/openoccd`，再用 `pgrep -af [o]penocd/[o]penoccd` 采集候选并过滤命令，只 kill 真实 `openocd`、`openoccd` 或 `timeout/sudo/env ... openocd`，避免误杀 `rtt_openocd_guard.py` 这类文件名含 openocd 的诊断脚本。已接入 `rtt_loop_rate_gate.py`、`rtt_usb_debug_snapshot.py`、`rtt_log_download_gate.py`、`rtt_param_persist_gate.py`、`rtt_acceptance_suite.py`、`rtt_sitl_on_hw_gate.py`。验证：`python3 -m py_compile` 覆盖新 guard 和 6 个接入脚本；RTT build_tests 共 7 个单测通过，其中包含“不要误杀 guard 脚本”的过滤测试；实机调用 `cleanup_openocd()` 返回结构化 JSON，并把当前 shell 中的 openocd 字符串作为 skipped 证据而非 kill 目标。远端 run `27921313319` 两个 matrix job 均 GREEN，日志显示 `Ran 7 tests` / `OK`，full 仍通过 `Binary integrity check PASSED`，APP_DESCRIPTOR hash 为 `0x06dea1c5`，未出现旧 SCons/CAN/hash=0 错误。
- `RTT-GAP-103`：新增 `Tools/scripts/rtt_usb_port_conflict_diag.py` 作为 Linux 主机侧 USB 端口占用冲突诊断。脚本只读系统状态，不 reset USB、不杀进程；它综合 `/dev/ttyACM*`、`/dev/ttyUSB*`、`/dev/serial/by-id`、pyserial、sysfs USB VID/PID、`lsof`、`fuser` 和 `pgrep`，标注 RTT MAVLink CDC、RTT SLCAN CDC、外接 USB 串口/CAN 调试器、STLink 等角色，并检测 QGroundControl、Mission Planner/Wine、slcand、ModemManager、brltty、openocd 是否占用或干扰端口。当前实机诊断 `rtt_gap103_usb_conflict_diag_20260622T000000Z/usb_port_conflict_diag.json` 给出 `GREEN`：`/dev/ttyACM1` 为 `rtt_mavlink_cdc`、`/dev/ttyACM2` 为 `rtt_slcan_cdc`，两者均未被占用，`/dev/ttyACM0` 为外接 `1a86_USB_Single_Serial` 调试器；单元测试、py_compile 均通过。远端 run `27920855999` 两个 matrix job 均 GREEN，日志显示 `Ran 4 tests` / `OK`，full 仍通过 `Binary integrity check PASSED`，APP_DESCRIPTOR hash 为 `0x0e56383d`，没有旧 SCons/CAN/hash=0 错误。
