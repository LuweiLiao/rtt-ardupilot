...
| RTT-GAP-101 | Test | FIXED | `rtt_acceptance_suite.py` 已新增 manifest 驱动的 `--full-chain --rounds N` 固定入口，串联 manifest 校验、QGC/MP host evidence、loop-rate、参数下载、MAVFTP、外设、日志、可选 SocketCAN/DroneCAN、source artifact audit，并输出 capability coverage matrix | `Tools/scripts/rtt_acceptance_suite.py`, `test_rtt_acceptance_suite.py`, `rtt_gap101_manifest_acceptance_suite_20260622T000000Z/mock_full_chain_acceptance.json`, `script_tests.log` | 后续实机总验收运行 `--full-chain --rounds N --include-socketcan --sudo-socketcan` 采集真硬件 N 轮 |
